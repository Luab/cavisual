    ...
    for x, _ in tcav_bar:
        ...
        if k in class_list:
            derivatives[k].append(
                directional_derivative(model, cav, layer_name, k))
...
class TCAV(object):
    ...
    def generate_activations(self, layer_names):
        for concept_name, data_loader in self.concept_dataloaders.items():
            self.activations[concept_name] = get_activations(
                self.model, self.output_dir, data_loader, concept_name,
                layer_names, self.max_samples, self.save_activations)

    def load_activations(self):
        for concept_name in self.concepts:
            self.activations[concept_name] = load_activations(
                os.path.join(self.output_dir,
                             "activations_%s.h5" % concept_name))

    def generate_cavs(self, layer_name):
        self.cav_trainer = CAV(self.concepts, layer_name, self.lr, self.model_type)
        self.cav_trainer.train(self.activations)
        self.cavs = self.cav_trainer.get_cav()
