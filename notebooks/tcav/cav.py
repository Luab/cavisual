...
def flatten_activations_and_get_labels(concepts, layer_name, activations):
    ...
    min_num_samples = np.min(
        [activations[c][layer_name].shape[0] for c in concepts])
    # flatten the activations and mark the concept label
    data = []
    concept_labels = np.zeros(len(concepts) * min_num_samples)
    for i, c in enumerate(concepts):
        data.extend(activations[c][layer_name][:min_num_samples].reshape(
            min_num_samples, -1))
...
class CAV(object):
    ...
    def train(self, activations):
        ...
        if len(self.model.coef_) == 1:
            self.cav = np.array([-self.model.coef_[0], self.model.coef_[0]])
        else:
            self.cav = -np.array(self.model.coef_)
